...
int main()
{
    ...
    while(1) {
    	...
    	enum rps_types fpga_move;
    	enum rps_types comp_move;
    	...
    	case ROCK:
    		sprintf(statement, "%s\n\r",
    				(comp_move == PAPER) ? "Computer wins! Paper > Rock" :
    											 "FPGA wins! Rock > Scissors");
    		break;

    	case PAPER:
    		sprintf(statement, "%s\n\r",
    				(comp_move == ROCK) ? "FPGA wins! Paper > Rock" :
    									  "Computer wins! Scissors > Paper");
    		break;

    	case SCISSOR:
    		sprintf(statement, "%s\n\r",
    				(comp_move == ROCK) ? "Computer wins! Rock > Scissors" :
